            ...
            while let Some(message) = self.rx.recv().await {
                ...
                match parsed_message {
                    ...
                    Message::Event(event) => {
                        self.event_tx
                            .send(event)
                            .expect("Failed to send event to event channel");
                    }
        ...
        tokio::spawn(async move {
            while let Some(event) = rx.recv().await {
                for listener in listeners.lock().await.iter_mut() {
                    listener
                        .send(event.clone())
                        .expect("Unable to send event to event channel");
                }
            }
        })
    }
            ...
            while let Some(message) = self.rx.recv().await {
                ...
                match parsed {
                    cdp_base::Message::CommandResponse(r) => {
                        let _ = self
                            .command_response_tx
                            .send(CdpCommandResponseState::Success(r));
                    }
                    cdp_base::Message::ErrorResponse(e) => {
                        tracing::debug!("[CdpListener]: Error Response: {}", e);
                        let _ = self
                            .command_response_tx
                            .send(CdpCommandResponseState::Error(e));
                    }
