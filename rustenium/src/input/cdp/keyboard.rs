...
impl CdpKeyboard {
    pub fn new(
        ...
        Self { session, modifiers, pressed_keys: HashSet::new() }
    }
    ...
    pub async fn down(&mut self, key: &str) -> Result<(), InputError> {
        let def = key_definition(key).ok_or_else(|| InputError::UnknownKey(key.to_string()))?;

        let auto_repeat = self.pressed_keys.contains(def.code);
        self.pressed_keys.insert(def.code.to_string());

        let shift_active = *self.modifiers.lock().unwrap() & 8 != 0;
        let effective_key = if shift_active { def.shift_key.unwrap_or(def.key) } else { def.key };
        *self.modifiers.lock().unwrap() |= Self::modifier_bit(effective_key);
        ...
        let text = if shift_active { def.shift_text.or(def.text) } else { def.text };
        ...
        let cmd = builder.build().map_err(InputError::BuildError)?;
    ...
    /// Release a key.
    pub async fn up(&mut self, key: &str) -> Result<(), InputError> {
        let def = key_definition(key).ok_or_else(|| InputError::UnknownKey(key.to_string()))?;

        let shift_active = *self.modifiers.lock().unwrap() & 8 != 0;
        let effective_key = if shift_active { def.shift_key.unwrap_or(def.key) } else { def.key };
        *self.modifiers.lock().unwrap() &= !Self::modifier_bit(effective_key);
        self.pressed_keys.remove(def.code);
    ...
    pub async fn press(&mut self, key: &str, delay_ms: u64) -> Result<(), InputError> {
        self.down(key).await?;
        if delay_ms > 0 {
    ...
    pub async fn type_text(&mut self, text: &str, delay_ms: u64) -> Result<(), InputError> {
