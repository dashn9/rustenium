...
pub async fn test_navigate_with_wait_complete(mut browser: impl BidiBrowser) {
    let result = browser
        .navigate_with_options("https://example.com", nav_opts())
        .await;
    assert!(result.is_ok());
    browser.close().await.unwrap();
}
...
pub async fn test_navigate_multiple_pages(mut browser: impl BidiBrowser) {
    browser
        ...
        .unwrap();
    browser
        .navigate_with_options("https://httpbin.org/html", nav_opts())
        .await
        .unwrap();
    browser.close().await.unwrap();
}
...
pub async fn test_find_nodes_nonexistent_returns_empty(mut browser: impl BidiBrowser) {
    browser
        ...
        .unwrap();
    let nodes = browser
        .find_nodes(css!("#nonexistent-element-xyz"))
        .await
        .unwrap();
    assert!(
        nodes.is_empty(),
        "Should return empty vec for nonexistent selector"
    );
    browser.close().await.unwrap();
}
...
pub async fn test_find_node_nonexistent_returns_none(mut browser: impl BidiBrowser) {
    browser
        ...
        .unwrap();
    let node = browser
        .find_node(css!("#nonexistent-element-xyz"))
        .await
        .unwrap();
    assert!(
        node.is_none(),
        "Should return None for nonexistent selector"
    );
    browser.close().await.unwrap();
}
...
pub async fn test_get_inner_text(mut browser: impl BidiBrowser) {
    ...
    assert!(
        ...
        "h1 should contain 'Example Domain', got: {}",
        text
    );
    browser.close().await.unwrap();
}
...
pub async fn test_get_text_content(mut browser: impl BidiBrowser) {
    ...
    assert!(
        text.contains("Example Domain"),
        "text_content should contain 'Example Domain'"
    );
    browser.close().await.unwrap();
}
...
pub async fn test_mouse_move_to_point(mut browser: impl BidiBrowser) {
    ...
    let result = mouse
        .move_to(Point { x: 100.0, y: 200.0 }, &context, Default::default())
        .await;
...
pub async fn test_evaluate_script_returns_number(mut browser: impl BidiBrowser) {
    browser
        ...
        .unwrap();
    let result = browser
        .evaluate_script("document.querySelectorAll('p').length".to_string(), false)
        .await
        .unwrap();
...
pub async fn test_get_active_context_id(browser: impl BidiBrowser) {
    ...
    assert!(
        context.is_ok(),
        "Should have an active context after creation"
    );
    browser.close().await.unwrap();
}
...
pub async fn test_emulate_timezone(mut browser: impl BidiBrowser) {
    browser
        ...
        .unwrap();
    browser
        .emulate_timezone(Some("America/New_York".to_string()))
        .await
        .unwrap();
    let eval = browser
        .evaluate_script(
